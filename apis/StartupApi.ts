...
export class StartupApiRequestFactory extends BaseAPIRequestFactory {
    ...
    /**
     */
    public async startupSendMailGet(_options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // Path Params
        const localVarPath = '/Startup/sendMail';
...
export class StartupApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to startupSendMailGet
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async startupSendMailGet(response: ResponseContext): Promise<void > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            return;
        }
